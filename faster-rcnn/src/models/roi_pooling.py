...
class RoIPooling(Layer):
    ...
    # Inputs are a list, [ x_maps, x_rois ], where x_maps and x_rois must have the same batch size, N. 
    # The first application of map_fn() iterates over N samples of [ x_map, x_roi ]. 
    # For this to work, the data type of the final tensor must be specified otherwise map_fn() apparently infers a
    ...
    def call(self, inputs):
        if self.pool_size == 7 and inputs[0].shape[3] == 512:
            ...
            return tf.map_fn(
                fn = lambda input_pair: RoIPooling._compute_pooled_rois_7x7x512(feature_map = input_pair[0], rois = input_pair[1]),
                elems = inputs,
            ...
            return tf.map_fn(
                fn = lambda input_pair: RoIPooling._compute_pooled_rois(feature_map = input_pair[0], rois = input_pair[1], pool_size = self.pool_size),
                elems = inputs,
                fn_output_signature = tf.float32  # this is absolutely required else the fn type inference seems to fail spectacularly
            )
    ...
    def _compute_pooled_rois(feature_map, rois, pool_size):
        # Given a feature map and its associated RoIs, iterate over all RoIs for this map. 
        # This is the second level of iteration and yields the num_rois dimension: (samples, num_rois, ...)
        return tf.map_fn(
            fn = lambda roi: RoIPooling._compute_pooled_roi(feature_map = feature_map, roi = roi, pool_size = pool_size),
            elems = rois,
    ...
    def _compute_pooled_roi(feature_map, roi, pool_size):
        ...
        # Given a feature map and a single RoI, computes the pooled map of shape (pool_size, pool_size).
    ...
    def _pool_one_cell(region_of_interest, pool_y_start, pool_x_start, y_step, x_step, region_height, region_width, pool_size, num_channels):
        ...
        # This function maps a single pooling cell over some part of the RoI and then computes the max 
        # of the RoI cells inside that pooling cell. The operation is performed per-channel, yielding a result of shape (1, num_channels).
