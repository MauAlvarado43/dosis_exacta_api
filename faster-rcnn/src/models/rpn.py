...
from src.data.math import tf_convert_deltas_to_boxes

class RegionProposalNetwork(Model):

    def __init__(self, C, max_proposals_pre_nms_train, max_proposals_post_nms_train, max_proposals_pre_nms_infer, max_proposals_post_nms_infer, l2 = 0, allow_edge_proposals = False):
        super().__init__()
        ...
        initial_weights = tf.keras.initializers.RandomNormal(mean = 0.0, stddev = 0.001, seed = None)

        anchors_per_location = C.num_anchors
    ...
    def call(self, inputs, training):
        ...
        # Keep only the top-N scores. Note that we do not care whether the proposals were labeled as objects
        # (socre > 0.5) and perform a simple ranking among all of them. Restricting them has a strong
        # advese impact on training performance
        sorted_indices = tf.argsort(objectness_scores) # Sorts in ascending order
        sorted_indices = sorted_indices[::-1] # Reverse to descending order
        proposals = tf.gather(proposals, indices = sorted_indices)[0:max_proposals_pre_nms] # grab the top-N best proposals
        objectness_scores = tf.gather(objectness_scores, indices = sorted_indices)[0:max_proposals_pre_nms] # grab the top-N best scores
    ...
    def _extract_valid(self, anchor_map, anchor_valid_map, objectness_score_map, box_delta_map, allow_edge_proposals):
        ...
        # anchor_valid_map is (batch, height, width, num_anchors)
    ...
    # y_predicted is a tensor of shape (batch_size, height, width, num_anchors) containing objectness scores (0 = background, 1 = object).
    # gt_rpn_map GT tensor of shape (batch_size, height, width, num_anchors, 6).
    @staticmethod
    ...
    # y_predicted is a tensor of shape (batch_size, height, width, num_anchors, 4) containing RoI box delta regressions for each anchor, stored as: ty, tx, th, tw
    # gt_rpn_map GT tensor of shape (batch_size, height, width, num_anchors, 6)
    @staticmethod
    def regression_loss(y_predicted, gt_rpn_map):
        ...
        # Include only anchors that are used in the mini-batch and which correspond to objects (positive samples)
        ...
        # y_mask is of the wrong shape, so we need to repeat it 4 times to match y_predicted_regression
